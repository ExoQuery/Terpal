  ...
  inline fun <reified R: Any> map(crossinline f: (T) -> R): JdbcDecoder<R> =
    object: JdbcDecoder<R>() {
      ...
      override fun decode(session: Connection, row: ResultSet, index: Int) =
        this@JdbcDecoder.decode(session, row, index)?.let(f)
    }
  ...
  companion object {
    inline fun <reified T: Any> fromFunction(crossinline f: (Connection, ResultSet, Int) -> T?): JdbcDecoder<T> =
      object: JdbcDecoder<T>() {
        ...
        override fun decode(session: Connection, row: ResultSet, index: Int): T? = f(session, row, index)
      }
  }
}
...
abstract class JdbcDecodersBasic: SqlDecoders<Connection, ResultSet>() {
  ...
  override val DateDecoder: JdbcDecoder<java.util.Date> =
    JdbcDecoder.fromFunction { _, rs, i ->
      rs.getTimestamp(i, Calendar.getInstance(dateTimeZone))?.let { java.util.Date(it.getTime()) }
    }
}

open class JdbcDecodersWithTime(override val dateTimeZone: TimeZone, val additionalDecoders: Set<SqlDecoder<Connection, ResultSet, *>>): JdbcDecodersBasic() {
  ...
  override val decoders by lazy { super.decoders + setOf(SqlDateDecoder) }
}

open class JdbcDecodersWithTimeLegacy(override val dateTimeZone: TimeZone, val additionalDecoders: Set<SqlDecoder<Connection, ResultSet, *>>): JdbcDecodersBasic() {
  /** Convenience instance so you can reference decoders that are not dependent on dateTimeZone/additionalDecoders */
  companion object: JdbcDecodersWithTimeLegacy(TimeZone.getDefault(), emptySet())

  override val LocalDateDecoder: JdbcDecoder<LocalDate> = JdbcDecoder.fromFunction { _, rs, i -> rs.getDate(i)?.let { it.toLocalDate() } }
  override val LocalTimeDecoder: JdbcDecoder<LocalTime> = JdbcDecoder.fromFunction { _, rs, i -> rs.getTime(i)?.let { it.toLocalTime() } }
  override val LocalDateTimeDecoder: JdbcDecoder<LocalDateTime> = JdbcDecoder.fromFunction { _, rs, i -> rs.getTimestamp(i).let { it.toLocalDateTime() } }
  override val ZonedDateTimeDecoder: JdbcDecoder<ZonedDateTime> = JdbcDecoder.fromFunction { _, rs, i -> rs.getTimestamp(i).let { ZonedDateTime.ofInstant(it.toInstant(), dateTimeZone.toZoneId()) }  }
  val sqlDateEncoder = JdbcDecoder.fromFunction { _, rs, i -> rs.getDate(i) }

  override val InstantDecoder: JdbcDecoder<Instant> = JdbcDecoder.fromFunction { _, rs, i -> rs.getTimestamp(i).let { it.toInstant() } }
  override val OffsetTimeDecoder: JdbcDecoder<OffsetTime> = JdbcDecoder.fromFunction { _, rs, i -> rs.getTime(i)?.let { OffsetTime.of(it.toLocalTime(), ZoneOffset.UTC) } }
  override val OffsetDateTimeDecoder: JdbcDecoder<OffsetDateTime> = JdbcDecoder.fromFunction { _, rs, i -> rs.getTimestamp(i).let { OffsetDateTime.ofInstant(it.toInstant(), dateTimeZone.toZoneId()) } }
