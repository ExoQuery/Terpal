...
class EncodingSpec: FreeSpec({
  ...
  val ctx by lazy {
    object: TerpalContext.SqlServer(ds) {
  ...
  "Encode/Decode Other Time Types ordering" {
    ...
    // SQL Server seems to be extremely finnicky about (order-based) comparison of the TIME types)
    val actual =
      Sql("""
          SELECT * FROM TimeEntity 
          WHERE 
            sqlDate > ${timeEntityA.sqlDate} 
            AND sqlTime > CAST(${timeEntityA.sqlTime} AS TIME)
            AND sqlTimestamp > ${timeEntityA.sqlTimestamp}
            AND timeLocalDate > ${timeEntityA.timeLocalDate}
            AND timeLocalTime > CAST(${timeEntityA.timeLocalTime} AS TIME)
